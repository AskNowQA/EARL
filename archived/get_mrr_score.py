        ...
        for k in range(len(dat)):
            ...
            for lists in list_dat:
               if lists['correctlabel'] == 1:
                    #correct_labels.append([lists['connections'], lists['sumofhops'], lists['lengthpathmetric']])
                    correct_labels.append([lists['connections'], lists['sumofhops']])
               else:
                    #incorrect_labels.append([lists['connections'], lists['sumofhops'], lists['lengthpathmetric']])
...
def get_vecs_von_test(test_list):
    ...
    for lists in test_list:
        #vec.append([lists['connections'], lists['sumofhops'], lists['lengthpathmetric']])
...
def get_mrr_test(dat, model):
	...
	for d in range(len(list_dat)):
        	list_dat[d]['prediction'] = predictions[d]
...
def get_mrr(lists):
	...
	#put elastic search == 0 while doing for elastic search only
