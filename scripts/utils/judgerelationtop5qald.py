...
for item in d1['questions']:
    ...
    resources = re.findall('dbo:([^\s].*) ', sparql, re.IGNORECASE)
    itarr = []
    for resource in resources:
        itarr.append('http://dbpedia.org/ontology/'+resource.split(' ')[0])
    entities = re.findall('<http://dbpedia.org/ontology/([^\s].*)>', sparql, re.IGNORECASE)
    for entity in entities:
        itarr.append('http://dbpedia.org/ontology/'+entity.split('>')[0])
    resources = re.findall('dbp:([^\s].*) ', sparql, re.IGNORECASE)
    for resource in resources:
        itarr.append('http://dbpedia.org/property/'+resource.split(' ')[0])
    entities = re.findall('<http://dbpedia.org/property/([^\s].*)>', sparql, re.IGNORECASE)
    for entity in entities:
        itarr.append('http://dbpedia.org/property/'+entity.split('>')[0])
    print(itarr)
