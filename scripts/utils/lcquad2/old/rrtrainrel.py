...
_zcount = 0
for item in d:
    if item['truerelation'] == 1:
        inputs.append([item['connections'],item['esrank'],item['sumofhops']])
        outputs.append(1.0)
        ocount += 1
    if item['truerelation'] == 0 and item['trueentity'] == -1:
        if zcount%80 == 0:
            inputs.append([item['connections'],item['esrank'],item['sumofhops']])
            outputs.append(0.0)
            _zcount += 1
...
for item in d1:
    ...
    _rels = re.findall( r'wdt:(.*?) ', item['sparql_wikidata'])
    for rel in _rels:
        itarr.append(rel)
...
while 1:
    ...
    tlqi = 0
    with torch.no_grad():
        for lcqitem,item in zip(lcqgold,d2):
    ...
    if true > besttrue:
        print('saved')
        torch.save(model.state_dict(), 'wikirerankerrelation.model')
        besttrue = true
