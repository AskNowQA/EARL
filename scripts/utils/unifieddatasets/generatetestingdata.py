...
dlcqtrain = json.loads(open('unifiedtest.json').read())
dquerytrain = json.loads(open('simpleentembedtest1.json').read())
...
for gold,query in zip(dlcqtrain,dquerytrain):
    try:
        ...
        if len(query) == 0:
            continue
        goldents = gold['entities']#re.findall( r'wd:([Q][0-9]*)', gold['sparql_wikidata'])
